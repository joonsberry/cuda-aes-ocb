...
#include <stdlib.h>
#include <sys/time.h>
#include "../OCB-AES/ocb.h" // find at https://github.com/DesWurstes/OCB-AES
...
void generateString(unsigned char *buf, int stringlen) {
...
int main(void) {
  ...
  unsigned char *message = malloc(fsize + 1);
  fread(message, 1, fsize, m_file);

  fgets(key, KEY_SIZE, k_file);
  ...
  sprintf(nonce, "abcdefghijklmn");

  printf("Key: %s\n", key);
  
  gettimeofday(&start, NULL);

  ocb_encrypt(key, nonce, NONCE_SIZE, message, message_size, "", 0, ciphertext);
  ...
  ocb_decrypt(key, nonce, NONCE_SIZE, ciphertext, message_size, "", 0, result);
